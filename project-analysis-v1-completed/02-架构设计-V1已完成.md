...
- REST + SSE
- JWT + RBAC + 租户隔离
- **集中配置**：`config/`（`@shellder/config`）统一 YAML + 环境变量，三端（server / worker / web）共享默认值
...
**说明**：**Policy / 策略引擎**、**审批中心**、**任务中心**、**Capability Routing**、**Audit** 管理面均已产品化（2026-06）；Skill 管理 UI 仍为实验中。
...
| Policy | `policy/` | 显式规则 CRUD、Policy 评估、`rule_hit` 留痕（管理菜单「策略引擎」） |
...
> 原设计为 Next.js；现网为 Vite + React Router，以代码为准。
...
| 层级 | V1 状态 |
|------|---------|
| 接入层（Web / Copilot / OpenAPI） | ✅ 三种入口均已验收 |
| 平台服务层 | ✅ 核心模块已落地；Capability Routing / Policy / Approval / Task 已产品化；Skill 管理 UI 实验中 |
| 知识与策略 | ✅ 知识（wiki 代理）；策略引擎独立菜单（`/rules`、`/rule-hits`）已产品化 |
| 治理层 | ✅ Auth / RBAC / Tenant / Audit；审批中心已产品化 |
| 工程划分 | ✅ `shellder-web-console`、`shellder-agent-server`、`shellder-job-worker` |
