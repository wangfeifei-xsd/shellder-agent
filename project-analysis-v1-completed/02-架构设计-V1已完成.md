...
**说明**：Policy、Approval、Audit、Task、Capability Routing、Skill 等模块在 server 内已实现，但对应管理菜单为「实验中」，图中未作为 V1 管理面交付。
...
2. **Agent Runtime** 装配上下文，调用 **Capability Routing**（实验中 UI，Runtime 内已用）得到能力类型
...
| Copilot | `copilot/` | 配置 CRUD + `/copilot/v1` BFF |
...
| §4.2 平台服务层 | ✅ 核心模块已落地；Task/Routing/Skill 管理 UI 实验中 |
