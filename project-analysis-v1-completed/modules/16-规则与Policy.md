...
| 属性 | 值 |
|------|-----|
| 菜单 | **策略引擎**（侧栏文案；RBAC key 仍为 `rule`，无「实验中」） |
| 路由 | `/rules`、`/rule-hits` |
...
位于 **能力路由** 之后、**Prompt 管理** 之前（与 `menu-items.tsx` 一致）。
...
RBAC：菜单 `rule`、模块 `rule.manage`（`auth/permissions.ts` 标签同为「策略引擎」）。
...
| 类型 | 表 | 管理菜单 | 命中留痕 |
|------|-----|----------|----------|
| **Policy 显式规则** | `rule` | 策略引擎 / 规则配置 | `rule_hit`（`rule_id` 有值） |
...
| 历史 | 现网 |
|------|------|
| 菜单在「知识库管理与规则」下 | **知识库** 与 **策略引擎** 拆为独立顶级菜单 |
| 侧栏文案「规则」、位于 OpenAPI 与审批之间 | **「策略引擎」**；位于 **能力路由** 与 **Prompt 管理** 之间 |
| 标注实验中 | **已去标注并产品化** |
