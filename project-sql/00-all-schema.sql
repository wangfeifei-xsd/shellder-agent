...
CREATE TABLE `agent_platform`.`tool` (
    ...
    `type`             ENUM('query', 'action', 'workflow', 'notification', 'http_query') NOT NULL
                       COMMENT '类型：查询型 / 操作型 / 流程型 / 通知型 / HTTP业务查询',
    `status`           ENUM('enabled', 'disabled') NOT NULL DEFAULT 'enabled' COMMENT '启用 / 停用',
    ...
    `config`           JSON         NOT NULL COMMENT '类型相关配置：query→sql / action,notification→http / http_query→httpQuery / workflow→workflow',
    `created_at`       DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    `updated_at`       DATETIME(3)  NOT NULL COMMENT '更新时间',
