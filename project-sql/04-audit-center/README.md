...
实现 Audit 模块的采集与查询，支撑管理后台「审计中心」四菜单（功能清单 §1.9、架构 §4.2 Audit）：
...
- **风险动作审计**：聚合**只读视图**，不单独建第四套采集通道（执行计划 §3.4 / §5.2）。V1 聚合自 `tool_call_audit.high_risk = true`，14-审批中心 就绪后再 JOIN `approval`。
