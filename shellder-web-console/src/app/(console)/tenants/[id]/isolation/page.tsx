  ...
  return (
    <>
      <Breadcrumb
        className="mb-4"
        items={[
          { title: <Link href="/tenants">租户列表</Link> },
          { title: <Link href={`/tenants/${id}`}>租户详情</Link> },
          { title: '租户隔离配置' },
        ]}
      />
      <div className="mb-4 flex items-center gap-2">
        <Button type="text" icon={<LeftOutlined />} onClick={() => router.push(`/tenants/${id}`)} />
        ...
        <Card className="max-w-2xl">
          <Form form={form} layout="vertical" onFinish={handleFinish}>
            ...
            <Space>
              ...
              <Button onClick={() => router.push(`/tenants/${id}`)}>取消</Button>
            </Space>
          </Form>
        </Card>
      )}
