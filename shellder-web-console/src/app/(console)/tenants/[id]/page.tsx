...
export default function TenantDetailPage() {
  ...
  return (
    <>
      <Breadcrumb
        className="mb-4"
        items={[{ title: <Link href="/tenants">租户列表</Link> }, { title: tenant.name }]}
      />
      <div className="mb-4 flex items-center justify-between">
        ...
        <Space>
          <Button icon={<EditOutlined />} onClick={() => router.push(`/tenants/${id}/edit`)}>
            编辑
          </Button>
          <Button onClick={() => router.push(`/tenants/${id}/isolation`)}>租户隔离配置</Button>
        </Space>
      </div>
